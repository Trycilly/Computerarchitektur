...
void clock_gui_update(uint64_t tick_us)
{
    ...
    uint16_t color_h = (edit == CURSOR_EDIT_HOUR)   ? BLUE : GREEN;
    uint16_t color_m = (edit == CURSOR_EDIT_MINUTE) ? BLUE : GREEN;

#if SELECT_12HOURS == 1
    int display_hour = hours % 12;
    if (display_hour == 0) display_hour = 12;
    const char *am_pm = clock_time_is_pm() ? "PM" : "AM";

    snprintf(time, sizeof(time), "%2d", display_hour);
    gui_draw_string(78, 180, time, &Font24, color_h, WHITE);

    gui_draw_string(112, 180, ":", &Font24, WHITE, WHITE);

    snprintf(time, sizeof(time), "%2d", minutes);
    gui_draw_string(129, 180, time, &Font24, color_m, WHITE);

    snprintf(time, sizeof(time), "%s", am_pm);
    gui_draw_string(164, 180, time, &Font24, color_h, WHITE);
#else
    snprintf(time, sizeof(time), "%2d", hours);
    gui_draw_string(78, 180, time, &Font24, color_h, WHITE);
    ...
    // Clock hands
    int hours_for_hand = hours % 12;
    int hours_arr = hours_for_hand * 8 + (minutes * 16) / 120;
    ...
    snprintf(time, sizeof(time), "%s", clock_time_get_timezone_name());
    gui_draw_string(108, 205, time, &Font16, WHITE, WHITE);
